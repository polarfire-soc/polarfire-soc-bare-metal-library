            ...
            case LPDDR4:
                {
                    ...
                    /*
                     *  lane out of sync on read- In the FIFO, no hang
                     *  Each lane has its own FIFO
                     *  Read point only increments once write has taken place
                     *
                     */
                    CFG_DDR_SGMII_PHY->rpc166.rpc166 = RPC_OVERRIDE_166_LANE_FIFO;
    ...
    write_calibration_using_mtc(uint8_t number_of_lanes_to_calibrate)
{
    ...
    uint32_t cal_data;
    uint64_t start_address = 0x0000000000001000ULL;
    uint32_t size = 0x8U;  /* Number of reads for each iteration */
