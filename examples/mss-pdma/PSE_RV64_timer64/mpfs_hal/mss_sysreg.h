...
#define CLOCK_CONFIG_CR_OFFSET   0x8
    /* "Sets the master synchronous clock dividerbits [1:0] CPU clock divider           (Reset=/1 =0)bits [3:2] AXI clock divider            (Reset=/1 =0)bits [5:4] AHB/APB clock divider  (Reset=/2 =1)00=/1   01=/2  10=/4 11=/8  (AHB/APB divider may not be set to /1)Note at reset MSS corner clock is 80MHz therefore divider is set to divide by 1"*/
...
#define ENVM_CR_OFFSET   0xB8
    ...
    /* "Sets the duration of the timer used to detect a non response of slow response from the PNVM on C and R bus accesses.Timer Duration = Value *  (1000/AHBFREQMHZ)   0x00: Timer dissabled. If the timer expires the AHB cycle is terminates using the HRESP protocol"*/
