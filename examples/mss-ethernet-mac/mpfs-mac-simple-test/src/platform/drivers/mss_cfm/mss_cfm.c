    ...
    )
{
    ...
    if(refsel0)
        CFM_REG->clkselReg |= (refsel0 << CFM_CLK_REFSEL0SHIFT);

    if(refsel1)
        CFM_REG->clkselReg |= (refsel1 << CFM_CLK_REFSEL1SHIFT);

    if(monSEL)
        CFM_REG->clkselReg |=(monSEL << CFM_CLK_MONSEL_SHIFT);


    if(monEN)
        CFM_REG->clkselReg |= (monEN << CFM_CLK_MONEN_SHIFT);
