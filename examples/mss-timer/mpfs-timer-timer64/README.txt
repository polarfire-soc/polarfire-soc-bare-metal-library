...
in a 64-bit configuration. This example uses timer 1 configured as a
periodic timer to generate a 64-bit timer interrupt.
In this mode, the two 32-bit timers are used to realize a 64-bit timer. In 
64 bit mode updating the various “counter-value” registers requires two 32-bit 
writes which must be sequenced upper 32-bit word then lower 32-bit word. 
The writes only take effect when the second write (to the lower 32 bits) takes 
effect.
--------------------------------------------------------------------------------
...
at periodic time intervals. The messages are displayed when the timer interrupt 
occurs.
--------------------------------------------------------------------------------
                                Target hardware
--------------------------------------------------------------------------------
...
"mpfs-timer-timer64 renode hart0 debug.launch" which is configured for Renode.
Alternatively, you can perform above two steps in one go by launching
mpfs-timer-timer64 renode hart0 start-platform-and-debug.launch which
...
Build the project and launch the debug configuration named 
mpfs-timer-timer64 hw-emulation all-harts Debug.launch which is configured for 
